...
### **특정 범위의 정수**
IntStream과 LongStream은 다음과 같이 **지정된 범위의 연속된 정수**를 스트림으로 생성해서 반환하는 range()와 rangeClose()를 가지고 있다.
```java
...
```java
    ...
    ...등
```
- IntSummaryStatics는 위와 같이 다양한 종류의 메서드를 제공하며, 이중에 필요한 것을 골라서 사용하면 된다.
...
- 요소가 배열안 두 스트림을 합쳐서 문자열이 요소인 스트림 ```Stream<String>```을 만들고자 할 때 flatMap()을 사용하지 않고, map()을 사용하면, 스트림의 스트림으로 생성된다.
