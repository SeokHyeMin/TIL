...
### Heap sort
#### 힙이란?
> 힙이란 **최대값이나 최소값**을 찾아내는 연산을 빠르게 하기 위해 고안된 **완전 이진트리를 기본**으로한 자료구조이다.
...
#### **heap과 binarySearchTree의 차이점**
힙과 이진탐색트리 모두 이진트리라는 점에서 공통점을 가진다. 그렇다면 차이점을 알아보자.
- 노드값이 다소 다르게 구성된다.
- 힙은 각 노드의 값이 자식노드보다 크거나 작은반면, 이진탐색트리는 왼쪽 자식노드가 제일 작고 부모노드가 그 다음 크며 오른쪽 자식노드가 가장 큰 값을 가진다.
- 힙은 우선순위(키) 정렬에 강점을 가지며, 이진탐색트리는 탐색에 강점을 지닌다.
